...
# @app.function(image=image, gpu="A10G", volumes={"/d ata": volume, "/models": modal_volume}, timeout=60 * 60 * 3)
# def train():
    
#     from datetime import datetime
#     timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
#     log_dir = f'/models/tensorboard_logs/{timestamp}'
#     writer = SummaryWriter(log_dir)
#     writer = SummaryWriter(log_dir)
...
#         print(f"Epoch {epoch+1}/{num_epochs}, Loss: {avg_val_loss:.4f}, Validation Loss: {avg_val_loss:.4f}, Accuracy: {accuracy:.2f}%")
#         if accuracy > best_accuracy:
#             best_accuracy = accuracy        
#             torch.save(
#                 {
